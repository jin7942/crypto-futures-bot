...
| 비용 $0 | Claude CLI(Claude Code) 활용. API 종량제 없음 |
...
- `isolated` 세션: 매 실행마다 새 히스토리
- Claude CLI 경유: 기존 구독 활용, 추가 비용 없음
...
| 항목 | 월 비용 |
|---|---|
| Claude CLI (Claude Code) | $0 (기존 구독 활용) |
| 외부 API (Fear & Greed, RSS) | $0 (무료) |
| **합계** | **$0/월** |
...
| Claude CLI 장애 | 센티먼트 생성 실패 | 기존 sentiment.json 유지, 4시간 후 neutral |
